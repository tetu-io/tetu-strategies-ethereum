...
import {IAaveProtocolDataProvider__factory, IAToken__factory,} from "../../../typechain";
...
async function main() {
  ...
  const dataProvider = IAaveProtocolDataProvider__factory.connect(MaticAddresses.AAVE_PROTOCOL_DATA_PROVIDER, signer);
  ...
  for (let i = 0; i < allLendingTokens.length; i++) {
    ...
    const aToken = IAToken__factory.connect(aTokenAdr, signer);
