...
interface IPlatformVotemarket {
  ...
  function claim(uint256 bountyId, address _recipient)
  external
  returns (uint256);
  ...
  function claimFor(address user, uint256 bountyId)
  external
  returns (uint256);

  function claimable(address user, uint256 bountyId)
  external
  view
  ...
  function getActivePeriod(uint256 bountyId)
  external
  view
  returns (Platform.Period memory);

  function getActivePeriodPerBounty(uint256 bountyId)
  external
  view
  returns (uint8);

  function getBlacklistedAddressesPerBounty(uint256 bountyId)
  external
  view
  ...
  external
  view
  returns (Platform.Bounty memory);

  function getCurrentPeriod() external view returns (uint256);

  function getPeriodsLeft(uint256 bountyId)
  external
  view
  returns (uint256 periodsLeft);

  function getUpgradedBountyQueued(uint256 bountyId)
  external
  view
  returns (Platform.Upgrade memory);
